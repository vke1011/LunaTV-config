# 源接口健康检测报告

最近更新时间：2026-08-26 08:19 CST

**总源数:** 68 | **检测关键词:** 美女 | **中转站:** `https://api.998836.xyz/?url=` (2 个域名)

| 状态 | 资源名称 | 地址 | API | 连接 | 搜索 | 成功 | 失败 | 成功率 | 最近7天趋势 |
|------|---------|-----|-----|:----:|---------|---------:|--------:|-------:|--------------|
| 🚨 | 🔞百万 | [Link](https://bwzy.tv) | [Link](https://bwzy.tv/api.php/provide/vod) | 🌐 | ❌ | 23 | 7 | 76.7% | ✅✅❌❌❌❌❌ |
| ✅ | 🎬如意 | [Link](https://www.ryzyw.com) | [Link](https://cj.rycjapi.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬爱奇艺 | [Link](https://iqiyizyapi.com) | [Link](https://iqiyizyapi.com/api.php/provide/vod) | 🌐 | ✅ | 29 | 1 | 96.7% | ✅✅✅✅❌✅✅ |
| ✅ | 🎬爱坤 | [Link](https://ikunzy.com) | [Link](https://ikunzyapi.com/api.php/provide/vod) | 🌐 | ✅ | 29 | 1 | 96.7% | ✅✅✅✅❌✅✅ |
| ✅ | 🎬猫眼 | [Link](https://www.maoyanzy.com) | [Link](https://api.maoyanapi.top/api.php/provide/vod) | 🌐 | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬优质 | [Link](https://1080zyk4.com) | [Link](https://api.yzzy-api.com/inc/apijson.php) | 🌐 | ✅ | 27 | 3 | 90.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬360 | [Link](https://360zy.com) | [Link](https://360zyzz.com/api.php/provide/vod) | 🌐 | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬魔都 | [Link](https://www.moduzy.net) | [Link](https://www.mdzyapi.com/api.php/provide/vod) | 🌐 | 无结果 | 27 | 3 | 90.0% | ✅❌❌✅✅✅✅ |
| ✅ | 🎬最大 | [Link](https://zuida.xyz) | [Link](https://api.zuidapi.com/api.php/provide/vod) | 🌐 | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬无尽 | [Link](https://wujinzy.com) | [Link](https://api.wujinapi.me/api.php/provide/vod) | 🌐 | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬速播 | [Link](https://www.subozy.com) | [Link](https://subocaiji.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅❌✅✅✅✅✅ |
| ✅ | 🎬新浪 | [Link](https://xinlangzy.com) | [Link](https://api.xinlangapi.com/xinlangapi.php/provide/vod) | 🌐 | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬极速 | [Link](https://jszyapi.com) | [Link](https://jszyapi.com/api.php/provide/vod) | 🌐 | ✅ | 30 | 0 | 100.0% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬西瓜 | [Link](https://xgzy1.tv) | [Link](https://caiji.xgzyapi.com/api.php/provide/vod) | 🌐 | 验证码 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬红牛 | [Link](https://www.hongniuzy.com) | [Link](https://www.hongniuzy2.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅❌✅✅✅ |
| ✅ | 🎬虎牙 | [Link](https://huyazy.com) | [Link](https://www.huyaapi.com/api.php/provide/vod) | 🌐 | ✅ | 26 | 4 | 86.7% | ✅✅✅❌✅✅✅ |
| ✅ | 🎬金鹰 | [Link](https://jinyingzy.com) | [Link](https://jyzyapi.com/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅❌✅✅✅ |
| ✅ | 🎬光速 | [Link](https://guangsuzy.com) | [Link](https://api.guangsuapi.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅❌✅✅✅ |
| ✅ | 🎬大众 | [Link](https://dzzy.com) | [Link](https://cdn.dzzyapi.com/api.php/provide/vod) | 🌐 | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬电影天堂 | [Link](http://caiji.dyttzyapi.com) | [Link](http://caiji.dyttzyapi.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬非凡 | [Link](https://cj.ffzyapi.com) | [Link](https://api.ffzyapi.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬暴风 | [Link](https://bfzy.tv) | [Link](https://bfzyapi.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬U酷 | [Link](https://www.ukuzy.com) | [Link](https://api.ukuapi88.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬豪华 | [Link](https://www.haohuazy.com) | [Link](https://hhzyapi.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬量子 | [Link](https://lzizy.com) | [Link](https://cj.lzcaiji.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬百度云 | [Link](https://bdzy1.com) | [Link](https://api.apibdzy.com/api.php/provide/vod) | 🔄 | ✅ | 26 | 4 | 86.7% | ✅✅✅✅❌❌✅ |
| ✅ | 🎬豆瓣 | [Link](https://dbzy.tv) | [Link](https://caiji.dbzy5.com/api.php/provide/vod) | 🌐 | 不支持 | 27 | 3 | 90.0% | ✅✅✅❌✅✅✅ |
| ✅ | 🎬茅台 | [Link](https://mtzy.me) | [Link](https://caiji.maotai999.vip/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬艾旦 | [Link](https://lovedan.net) | [Link](https://lovedan.net/api.php/provide/vod) | 🔄 | ✅ | 25 | 5 | 83.3% | ✅❌✅✅❌❌✅ |
| ✅ | 🎬天涯 | [Link](https://tyyszy.com) | [Link](https://tyyszyapi.com/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬快车 | [Link](https://kuaichezy.com) | [Link](https://caiji.kuaichezy.org/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬索尼 | [Link](https://suonizy.net) | [Link](https://suoniapi.com/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬鸭鸭 | [Link](https://yayazy1.com) | [Link](https://cj.yayazy.net/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬闪电 | [Link](https://shandianzy.com) | [Link](https://xsd.sdzyapi.com/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬樱花 | [Link](https://yhzy.cc) | [Link](https://m3u8.apiyhzy.com/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬OK | [Link](https://okzyw.cc) | [Link](https://api.okzyw.net/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬无水印 | [Link](https://wsyzy.cc) | [Link](https://api.wsyzy.net/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🎬牛牛 | [Link](https://niuniuzy.cc) | [Link](https://api.niuniuzy.me/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞155 | [Link](https://155zy2.com) | [Link](https://155api.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞极品 | [Link](https://jipinvip.com) | [Link](https://jipinvip1.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞玉兔 | [Link](https://yutuzy.com) | [Link](https://apiyutu.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞小鸡 | [Link](https://xjzy1.xyz) | [Link](https://api.xjzyapi.xyz/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞黄色仓库 | [Link](https://hsckzy888.com) | [Link](https://hsckzy888.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞大奶子 | [Link](https://apidanaizi.com) | [Link](https://apidanaizi.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞jkun | [Link](https://jkunzyapi.com) | [Link](https://jkunzyapi.com/api.php/provide/vod) | 🌐 | ✅ | 27 | 3 | 90.0% | ✅✅✅✅❌✅✅ |
| ✅ | 🔞奶香香 | [Link](https://Naixxzy.com) | [Link](https://Naixxzy.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞豆豆 | [Link](https://doudouzy.com) | [Link](https://api.douapi.cc/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞滴滴 | [Link](https://didiziyuan.com) | [Link](https://didiziyuan.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞黑料 | [Link](https://heiliaozy.cc) | [Link](https://www.heiliaozyapi.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞精品 | [Link](https://jingpinx.com) | [Link](https://jingpinx.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞CK | [Link](https://ckzy.me) | [Link](https://ckzy.me/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞搜AV | [Link](https://souavzyw.com) | [Link](https://api.souavzyw.net/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞番茄 | [Link](https://fqzy.me) | [Link](https://fqzy.me//api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞老色逼 | [Link](https://laosebizy8.com) | [Link](https://apilsbzy1.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞森林 | [Link](https://senlinzy1.com) | [Link](https://beiyong.slapibf.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞奥斯卡 | [Link](https://aosikazy6.com) | [Link](https://aosikazy6.com/api.php/provide/vod) | 🌐 | 不支持 | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞番号 | [Link](http://fhzy1.com) | [Link](http://fhapi9.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞AIvin | [Link](http://lbapiby.com) | [Link](http://lbapiby.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞杏吧 | [Link](http://www.sex8zy.com) | [Link](https://json.xingba222.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞麻豆 | [Link](https://91md.me) | [Link](https://91md.me/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞乐播 | [Link](https://lebozy.com) | [Link](https://lbapi9.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞辣椒 | [Link](https://lajiaozy.com) | [Link](https://apilj.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞鲨鱼 | [Link](https://shayuzy.com) | [Link](https://shayuzy.com/api.php/provide/vod) | 🌐 | ✅ | 22 | 8 | 73.3% | ✅✅✅✅✅❌✅ |
| ✅ | 🔞桃花 | [Link](https://thzy1.me) | [Link](https://thzy1.me/api.php/provide/vod) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞大地 | [Link](https://dadizy.com) | [Link](https://dadiapi.com/feifei) | 🌐 | ✅ | 28 | 2 | 93.3% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞细胞网 | [Link](https://www.xxibaozyw.com) | [Link](https://www.xxibaozyw.com/api.php/provide/vod) | 🌐 | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞香蕉 | [Link](https://www.xiangjiaozyw.com) | [Link](https://www.xiangjiaozyw.com/api.php/provide/vod) | 🌐 | ✅ | 29 | 1 | 96.7% | ✅✅✅✅✅✅✅ |
| ✅ | 🔞天美 | [Link](https://tmzyz.com) | [Link](https://tmzyz.com/api.php/provide/vod) | 🌐 | ✅ | 28 | 1 | 96.6% | ✅✅✅✅✅✅✅ |

<details>
<summary>📜 点击展开查看历史检测数据 (JSON)</summary>

```json
[
  {
    "date": "2026-07-26",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-26",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-27",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-28",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-29",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-30",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-07-31",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-01",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-02",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-03",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-04",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-05",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-07",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-08",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-09",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-10",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-11",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-12",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-13",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-14",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-17",
    "keyword": "美女",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-18",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-19",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-20",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-21",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-22",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-23",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-24",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-25",
    "keyword": "你好",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": true,
        "searchStatus": "❌"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  },
  {
    "date": "2026-08-26",
    "keyword": "美女",
    "results": [
      {
        "name": "🎬如意",
        "api": "https://cj.rycjapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱奇艺",
        "api": "https://iqiyizyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬爱坤",
        "api": "https://ikunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬猫眼",
        "api": "https://api.maoyanapi.top/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬优质",
        "api": "https://api.yzzy-api.com/inc/apijson.php",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬360",
        "api": "https://360zyzz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬魔都",
        "api": "https://www.mdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "无结果"
      },
      {
        "name": "🎬最大",
        "api": "https://api.zuidapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬无尽",
        "api": "https://api.wujinapi.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬速播",
        "api": "https://subocaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬新浪",
        "api": "https://api.xinlangapi.com/xinlangapi.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬极速",
        "api": "https://jszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬西瓜",
        "api": "https://caiji.xgzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "验证码"
      },
      {
        "name": "🎬红牛",
        "api": "https://www.hongniuzy2.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬虎牙",
        "api": "https://www.huyaapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬金鹰",
        "api": "https://jyzyapi.com/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬光速",
        "api": "https://api.guangsuapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬大众",
        "api": "https://cdn.dzzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬电影天堂",
        "api": "http://caiji.dyttzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬非凡",
        "api": "https://api.ffzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬暴风",
        "api": "https://bfzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬U酷",
        "api": "https://api.ukuapi88.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豪华",
        "api": "https://hhzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬量子",
        "api": "https://cj.lzcaiji.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🎬百度云",
        "api": "https://api.apibdzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬豆瓣",
        "api": "https://caiji.dbzy5.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬茅台",
        "api": "https://caiji.maotai999.vip/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬艾旦",
        "api": "https://lovedan.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": true,
        "searchStatus": "✅"
      },
      {
        "name": "🎬天涯",
        "api": "https://tyyszyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬快车",
        "api": "https://caiji.kuaichezy.org/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬索尼",
        "api": "https://suoniapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬鸭鸭",
        "api": "https://cj.yayazy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬闪电",
        "api": "https://xsd.sdzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬樱花",
        "api": "https://m3u8.apiyhzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬OK",
        "api": "https://api.okzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬无水印",
        "api": "https://api.wsyzy.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🎬牛牛",
        "api": "https://api.niuniuzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞155",
        "api": "https://155api.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞极品",
        "api": "https://jipinvip1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞玉兔",
        "api": "https://apiyutu.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞小鸡",
        "api": "https://api.xjzyapi.xyz/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黄色仓库",
        "api": "https://hsckzy888.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大奶子",
        "api": "https://apidanaizi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞jkun",
        "api": "https://jkunzyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奶香香",
        "api": "https://Naixxzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞豆豆",
        "api": "https://api.douapi.cc/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞滴滴",
        "api": "https://didiziyuan.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞黑料",
        "api": "https://www.heiliaozyapi.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞精品",
        "api": "https://jingpinx.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞CK",
        "api": "https://ckzy.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞搜AV",
        "api": "https://api.souavzyw.net/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞百万",
        "api": "https://bwzy.tv/api.php/provide/vod",
        "disabled": false,
        "success": false,
        "viaProxy": false,
        "searchStatus": "❌"
      },
      {
        "name": "🔞番茄",
        "api": "https://fqzy.me//api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞老色逼",
        "api": "https://apilsbzy1.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞森林",
        "api": "https://beiyong.slapibf.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞奥斯卡",
        "api": "https://aosikazy6.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "不支持"
      },
      {
        "name": "🔞番号",
        "api": "http://fhapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞AIvin",
        "api": "http://lbapiby.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞杏吧",
        "api": "https://json.xingba222.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞麻豆",
        "api": "https://91md.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞乐播",
        "api": "https://lbapi9.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞辣椒",
        "api": "https://apilj.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞鲨鱼",
        "api": "https://shayuzy.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞桃花",
        "api": "https://thzy1.me/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞大地",
        "api": "https://dadiapi.com/feifei",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞细胞网",
        "api": "https://www.xxibaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞香蕉",
        "api": "https://www.xiangjiaozyw.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      },
      {
        "name": "🔞天美",
        "api": "https://tmzyz.com/api.php/provide/vod",
        "disabled": false,
        "success": true,
        "viaProxy": false,
        "searchStatus": "✅"
      }
    ]
  }
]
```
</details>
